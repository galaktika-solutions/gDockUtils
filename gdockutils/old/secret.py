...
from . import (
    SecretAlreadyExists,
    SecretDatabaseNotFound,
    SecretDoesNotExist,
    uid,
    gid,
    SECRET_DATABASE_FILE,
)
...
def createsecret(secret, fromfile=None, random=None, value=None, force=None):
    try:
        readsecret(secret)
    ...
    if fromfile is not None:
        ...
        val = "".join(
            rnd.choice(string.ascii_letters + string.digits + string.punctuation)
...
def readsecret(secret, store=None, decode=False, secret_dir=""):
    try:
        with open(SECRET_DATABASE_FILE, "r") as db:
            secretlines = db.readlines()
