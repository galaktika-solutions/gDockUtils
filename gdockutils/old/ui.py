...
from .secret import readsecret, SecretDoesNotExist, createsecret, existing
from .prepare import defined_secrets


def ask(options=[], prompt="", default=None, multiple=False, yesno=False, marks=[]):
    """
...
def backup_ui():
    parser = argparse.ArgumentParser(
        description=("Perform a backup by asking the user for parameters.")
    )
    parser.add_argument(
        "-d", "--database", help="offer database type backup", action="store_true"
    )
    parser.add_argument(
        "-f", "--files", help='offer "files" type backup', action="store_true"
    )
    ...
    if len(types) == 1:
        ...
        typ = ask(types, prompt="What would you like to backup?", multiple=True)

    database_format = None
    if args.database:
        database_format = ask(
            ["custom", "plain"], prompt="Which db backup format do you want to use?"
        )

    backup(database_format, "files" in typ, args.backup_uid, args.backup_gid)


def restore_ui():
    parser = argparse.ArgumentParser(
        description=("Perform a restore by asking the user for parameters.")
    )
    parser.add_argument(
        "-d", "--database", help="offer database type restore", action="store_true"
    )
    parser.add_argument(
        "-f", "--files", help='offer "files" type restore', action="store_true"
    )
    parser.add_argument("--drop_db", help="the database to drop")
    parser.add_argument("--create_db", help="the database to create")
    parser.add_argument("--owner", help="the owner of the created database")
    ...
    if len(types) == 1:
        ...
        typ = ask(types, prompt="What would you like to restore?", multiple=True)
    ...
    if "database" in typ:
        ...
        entries = sorted(
            [e for e in entries if os.path.isfile(os.path.join(db_backup_dir, e))]
        )
        db_backup_file = ask(
            entries, prompt="Which db backup file would you like to use?"
        )

    restore(db_backup_file, "files" in typ, args.drop_db, args.create_db, args.owner)
...
def createsecret_ui():
    ...
    for i, s in enumerate(possible_secrets):
        try:
            ...
            marks.append(i)
    secret = ask(
        possible_secrets, prompt="Which secret would you like to create?", marks=marks
    )
    try:
        readsecret(secret)
    except SecretDoesNotExist:
        ...
        if not ask(prompt=prompt, yesno=True, default="n"):
            return

    modes = [("r", "create a random string"), ("v", "type it in the terminal")]

    entries = []
    if os.path.isdir(SECRET_SOURCE_DIR):
        entries = os.listdir(SECRET_SOURCE_DIR)
        entries = sorted(
            [e for e in entries if os.path.isfile(os.path.join(SECRET_SOURCE_DIR, e))]
        )
        if entries:
...
def readsecret_ui():
    ...
    if not secrets:
        return
    secret = ask(secrets, prompt="Select a secret")
