...
from . import get_param, uid, gid, printerr, run, cp, set_files_perms, set_backup_perms
from . import (
    POSTGRESCONF_ORIG,
    PG_HBA_ORIG,
    BACKUP_DIR,
...
def ensure_db(db_name=DATABASE_NAME, django_username=DATABASE_USER):
    ...
    dbpass_django = "'md5%s'" % md5(dbpass + django_username)
    ...
    run(
        [
            ...
            "postgres",
            "-c",
            "ALTER ROLE postgres ENCRYPTED PASSWORD %s" % dbpass_postgres,
            "-c",
            "CREATE ROLE %s" % django_username,
            "-c",
            "ALTER ROLE %s "
            "ENCRYPTED PASSWORD %s "
            "LOGIN SUPERUSER" % (django_username, dbpass_django),
            "-c",
            "CREATE ROLE explorer",
            "-c",
            "ALTER ROLE explorer " "ENCRYPTED PASSWORD %s LOGIN" % dbpass_explorer,
            "-c",
            "\c postgres %s" % django_username,
            "-c",
            "CREATE DATABASE %s" % db_name,
            "-c",
            "\c %s %s" % (db_name, django_username),
            "-c",
    ...
    gprun(
        userspec="postgres",
        sys_exit=False,
        command=["pg_ctl", "stop", "-s", "-w", "-m", "fast"],
    )
    set_files_perms()


def wait_for_db(silent=not DEBUG, noprint=False):
    while True:
        env = get_db_env()
        try:
...
def backup(database_format=None, files=None, backup_uid=None, backup_gid=None):
    ...
    if database_format:
        ...
        filename = filename.format(prefix=BACKUP_FILE_PREFIX, timestamp=timestamp)
        ...
        if db_backup_file.endswith(".backup"):
            ...
            run(
                [
                    ...
                    "postgres",
                    "-c",
                    "DROP DATABASE %s" % drop_db,
                    "-c",
                    "CREATE DATABASE %s OWNER %s" % (create_db, owner),
                ],
